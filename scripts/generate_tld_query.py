...
def generate_tld_query(tld_count=50):
    try:
        ...
        if not top_tlds:
            print("[ERROR] No valid TLDs extracted.")
            return

        print(f"[INFO] Top {len(top_tlds)} TLDs: {', '.join(top_tlds)}")

        # Build the KQL query
        kql_query = f"""// Query generated by StormInt TLD Generator
...
"""
        ...
        with open("stormint_tld_query.kql", "w") as f:
            f.write(kql_query)
            print("[INFO] Query saved to stormint_tld_query.kql")
